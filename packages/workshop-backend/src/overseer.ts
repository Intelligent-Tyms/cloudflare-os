...
class OverseerImpl implements AgentHooks {
  ...
  async exportGadget(gadgetId: WorkpieceId, formatId: string, chatId?: number)
    ...
    if (!gadget) throw new Error("The App server stub is unavailable.");
    using exportGadget = gadget;
    let format = formats.find(candidate => candidate.id === formatId);
    if (!format) throw new Error(`This App does not support export format: ${formatId}`);

    if (format.mode === "server") {
      if (!handler) throw new Error("The App export handler is unavailable.");
        ...
        if (billing) {
          ...
          if (billing.freeDailyLlmCalls != null && this.ownerId) {
            ...
            if (getAiGatewayConfig(this.env) && !isFreePlanModel(aiModel.config.model)) {
              blockMessage = `${aiModel.profile.name} isn't included in the free plan. ` +
                  `Upgrade for access to every model.`;
            } else {
              ...
              if (!quota.withinLimits) {
                blockMessage = `You've reached the free plan's daily limit. Upgrade for a ` +
  ...
  async forgeRestoreStubForBinding(
    ...
    if (entry.type !== "workpiece" || !this.storage.gadgets.get(entry.id)) {
      throw new Error(
          `[restore] is only available on App bindings; "${bindingName}" is not an App.`);
    }
...
export class OverseerDurableObject extends DurableObject<Cloudflare.Env> {
  ...
  async startHook(hookId: number): Promise<{
    ...
    if (config.disabledGatekeepers.includes(vendorId) ||
        ambientGatekeeperMode(config, vendorId, this.env) === "disabled") {
