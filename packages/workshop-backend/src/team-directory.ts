...
export function hasTeamDirectory(env: Env): boolean {
...
async function call(env: Env, path: string, body?: object): Promise<TeamView> {
...
export function fetchTeam(env: Env): Promise<TeamView> {
  return call(env, "/team");
}

/** Create an invitation; the directory sends the invitation email. */
export function createTeamInvitation(
    env: Env, email: string, role: string, invitedBy: string): Promise<TeamView> {
  return call(env, "/team/invitations", {email, role, invitedBy});
}

/** Revoke a pending invitation. */
export function revokeTeamInvitation(env: Env, invitationId: string): Promise<TeamView> {
...
export function resendTeamInvitation(env: Env, invitationId: string): Promise<TeamView> {
...
export function removeTeamMember(env: Env, email: string, actor: string): Promise<TeamView> {
